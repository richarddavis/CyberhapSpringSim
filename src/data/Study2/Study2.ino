...
void loop()
{
  ...
  // The software either requested new data or is sending new values
  if (Serial.available() > 0) {
    ...
    if (inByte == 0xaa) {
      ...
      // This is a simple update request
      Serial.println(xh, 6);
      //Serial.write((byte *) &xh, 4);
      //Serial.write(0x0a);\
    } else if (inByte == 0xbb) {
      // This transmission contains new data
      //while (Serial.available()) {
      // read the incoming buffer:
      Serial.readBytesUntil(0xff, serialInputBuffer, lengthInputBuffer);
